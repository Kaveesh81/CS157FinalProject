...
public class JdbcProjectRepository implements ProjectRepository {
    ...
    private final RowMapper<Project> projectRowMapper = new RowMapper<Project>() {
        ...
        public Project mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            project.setProjectLeadId(rs.getLong("project_lead_id"));
    ...
    public Project save(Project project) {
        jdbcTemplate.update(
                "INSERT INTO projects (title, description, topic, semester_id, project_lead_id, github_link, spots_available, is_approved) "
                        +
                        "VALUES(?,?,?,?,?,?,?,?)",
                project.getTitle(), project.getDescription(), project.getTopic(),
                project.getSemesterId(), project.getProjectLeadId(), project.getGithubLink(),
                project.getSpotsAvailable(), project.isApproved());

        return jdbcTemplate.queryForObject(
                "SELECT p.*, s.term, s.year FROM projects p " +
                        "JOIN semesters s ON p.semester_id = s.semester_id " +
                        "WHERE p.title = ? AND p.project_lead_id = ?",
                projectRowMapper, project.getTitle(), project.getProjectLeadId());
    }
    ...
    public int update(Project project) {
        return jdbcTemplate.update(
                "UPDATE projects SET title=?, description=?, topic=?, semester_id=?, " +
                        "project_lead_id=?, github_link=?, spots_available=?, is_approved=? " +
                        "WHERE project_id=?",
                project.getTitle(), project.getDescription(), project.getTopic(),
                project.getSemesterId(), project.getProjectLeadId(), project.getGithubLink(),
